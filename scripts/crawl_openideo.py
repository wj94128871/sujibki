...
# -*- coding: utf-8 -*-
"""
OpenIDEO 사회 임팩트 챌린지 수집
- 목록: GET https://challenges.openideo.com/challenge/<slug> 패턴. 인덱스/아카이브 페이지
        https://openideo.com/challenges
- 상세: GET https://openideo.com/challenge/<slug> (HTTP 200, JS 일부)
- 봇: 없음(requests OK).
출력: crawled/openideo/
사용법: python crawl_openideo.py [--pages 5] [--max-detail 20]
"""
from __future__ import annotations
import argparse, json, os, re, time, sys, logging
import requests

BASE = "https://openideo.com"
LISTING = f"{BASE}/challenges"
...
log = logging.getLogger("openideo")
...
def extract_links(html: str) -> list[str]:
    cand = re.findall(r'href="(/challenge/[A-Za-z0-9_-]+)"', html)
    cand += re.findall(r'href="(/challenges/[A-Za-z0-9_-]+)"', html)
...
def main() -> None:
    ...
    ap.add_argument("--out", default="crawled/openideo")
    ...
    for p in range(1, args.pages + 1):
        u = LISTING + (f"?page={p}" if p > 1 else "")
        log.info("목록 %d %s", p, u)
