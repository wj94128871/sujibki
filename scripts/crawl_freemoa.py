...
def main():
    ...
    ap.add_argument("--cookies", default="crawled/freemoa_oauth/cookies_freemoa.json")
    ...
    ap.add_argument("--out", default="crawled/freemoa_oauth")
    ap.add_argument("--delay", type=float, default=2.0)
    ap.add_argument("--date-from", default="", help="등록일자 시작 (YYYY-MM-DD, INS_TIME 기준)")
    ap.add_argument("--date-to", default="", help="등록일자 끝 (YYYY-MM-DD, INS_TIME 기준)")
    ...
    def in_range(ins_time):
        if not ins_time or not (df or dt):
            return True
        d = str(ins_time)[:10].replace("-", "")
        if df and d < df:
            return False
        if dt and d > dt:
            return False
        return True
    max_pages = args.max_pages if args.max_pages > 0 else args.pages
    for page in range(1, max_pages + 1):
        keep, raw_n, tot = get_list(s, page, args.out, balanced=args.balanced)
        keep = [it for it in keep if in_range(it.get("INS_TIME"))]
        print(f"[목록 {page}] 수신 {raw_n}건 → 도급필터 {len(keep)}건 (날짜범위 {args.date_from}~{args.date_to})")
        if page == 1:
            meta["total_rows"] = tot
        for it in keep:
            ...
            if pid:
                all_ids.append(pid)
        # 날짜 경계 판단: 이 페이지 항목이 모두 범위 이전이면 중단
        raw_keep, _, _ = get_list(s, page, None, balanced=args.balanced)
        times = [it.get("INS_TIME", "")[:10] for it in raw_keep]
        if (df or dt) and times and all(t.replace("-", "") < df for t in times):
