...
def extract_ids_with_dates(html):
    """카드별 project id + 등록일자 매핑 (목록 페이지용)."""
    out = []
    for m in re.finditer(r'등록일자\s*([\d.]+)', html):
        seg = html[max(0, m.start() - 4000):m.start()]
        pids = re.findall(r'/project/(\d+)/', seg)
        if pids:
            out.append({"id": pids[-1], "date": m.group(1).rstrip('.')})
    # 중복 id 제거 (마지막 항목 유지)
    seen = {}
    for o in out:
        seen[o["id"]] = o
...
def main():
    ...
    ap.add_argument("--cookies", default="crawled/wishket_oauth/cookies_wishket.json")
    ...
    ap.add_argument("--out", default="crawled/wishket_oauth")
    ap.add_argument("--delay", type=float, default=5.0, help="요청 간 딜레이(초); 위시켓 crawl-delay=5")
    ap.add_argument("--date-from", default="", help="등록일자 시작 (YYYY.MM.DD, 목록 기준 필터)")
    ap.add_argument("--date-to", default="", help="등록일자 끝 (YYYY.MM.DD, 목록 기준 필터)")
    ap.add_argument("--max-pages", type=int, default=0, help="목록 스캔 최대 페이지 수 (0=자동: 날짜경계까지)")
    args = ap.parse_args()
    ...
    if args.only_detail_ids:
        ...
        # 날짜 필터 준비
        df = args.date_from.replace(".", "")  # 20260801
        dt = args.date_to.replace(".", "")
        def in_range(date_str):
            if not date_str or not (df or dt):
                return True
            d = date_str.replace(".", "")
            if df and d < df:
                return False
            if dt and d > dt:
                return False
            return True
        max_pages = args.max_pages if args.max_pages > 0 else args.list_pages
        for page in range(1, max_pages + 1):
            ...
            if not r:
                continue
            cards = extract_ids_with_dates(r.text)
            page_ids = [c["id"] for c in cards if in_range(c["date"])]
            all_dates = [c["date"] for c in cards]
            print(f"  카드 {len(cards)}개 | 날짜 {all_dates} | 필터통과 {len(page_ids)}개")
            priv = extract_private_ids(r.text)
            ids += page_ids
            meta["private_ids"] += priv
            # 날짜 필터가 켜져 있고, 이 페이지 전체가 범위 이전이면 중단
            if (df or dt) and all_dates and all(d and d.replace('.', '') < df for d in all_dates):
                print(f"  [중단] 페이지 {page}부터 범위({args.date_from}) 이전 — 스캔 종료")
                break
            time.sleep(args.delay)
        ids = list(dict.fromkeys(ids))
    meta["ids"] = ids

    # 상세 수집 (only-detail-ids 모드는 전부 수집)
    detail_rows = []
    if args.only_detail_ids:
        targets = ids
    else:
